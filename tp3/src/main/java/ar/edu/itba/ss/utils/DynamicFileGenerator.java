...
public class    DynamicFileGenerator {
    ...
    public void generateTempDymanicFile(int seed, int numberParticles, double side, double temp,
                                    double m1, double m2,
                                    double r1, double r2) {
        try(FileWriter fw = new FileWriter("frame-0.xyz", false);
            BufferedWriter bw = new BufferedWriter(fw);
            PrintWriter out = new PrintWriter(bw))
        {
            ...
            double auxX, auxY, auxAngle, auxV;
            double v = temp/numberParticles;
            double vx = v/Math.sqrt(2);
            double vy = v/Math.sqrt(2);
            ...
            for(int i = 0; i < numberParticles ; i++) {
                if(i==0){
                    ...
                    out.println(auxX + " " + auxY + " " + vx + " " + vy + " " + 0 + " " + r1);
                    particles.add(new Particle(i, auxX, auxY, v, vy,m1,r1));
                }
            }
            out.close();
        } catch (IOException e) {
